...
- PR #23 マージ。AC #1 / #4 の画面側の挙動はユーザーが実機で確認し OK。
- レビュー後の追加変更: 「メモリに保存」をテキストボタンから 24px のアイコンボタン (Lucide memory-stick を SVG 直書き、ISC notice を THIRD_PARTY_NOTICES.md §4 に追記) に変更。単独チャットのレビュー・コピーと同じ見え方に揃えるため。パッケージ依存は追加していない。
- Codex レビューは 4 ラウンド。ラウンド 2 の [P2] はフォーカス復帰 effect の依存を下書きにしたため入力 1 文字でフォーカスが飛ぶ実バグで、依存を開閉状態に変更して解消。Tab の閉じ込めも同時に入れた。
- 派生タスク: TASK-29 (多人数会話の発言にコピーボタン) を m-1 に起票。
<!-- SECTION:NOTES:END -->
