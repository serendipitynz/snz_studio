...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 発言が 0 件の多人数会話では、チャット画面からプリセットを選んで適用できる (同梱・インポート両方)
- [x] #2 適用後の参加者・ターンルール・場面設定が、作成時に同じプリセットを指定した場合と一致する
- [x] #3 発言が 1 件以上ある会話ではプリセット適用が UI に出ず、API も拒否する
...
## レビューで入った変更 (PR #17, Codex CLI 2 ラウンド)

いずれも「発言 0 件」という前提が、確認から置き換えまでのあいだに崩れうるという指摘。

- **[P2] ターン実行との競合** → `TurnEngine.WithTurnExcluded` を追加し、発言有無の確認を含む適用全体を
  ターンの chat 単位スロット (設計書 §4.2 手順 0) の中で行う。ターンは発言者を選んでから完了時に
  message を保存するので、排他の外で編成を物理削除すると、保存された message が既に消えた参加者を
  指し、話者を解決できない発言が残る。ターン実行中の適用は 409。
- **[P2] 人間の介入発言との競合** → `TurnEngine.WithMessageWrite` (chat 単位の書き込みロック) を追加し、
  適用と `storeHumanMessage` で共有する。ターンのスロットに寄せなかったのは、ターン中の介入が
  設計書 §4.4 の仕様であり、409 で拒否するのも数十秒の生成完了まで待たせるのも不可だから。
  ロック順序は ターンのスロット → 書き込みロックで、人間側は後者だけなので循環しない。
  `TestMessageWriteExclusion` / `TestMessageWriteIsNotTheTurnSlot` で両側を固定。

## UI の調整 (ユーザー指摘)

編成パネルのプリセット節を `<details>` にし、既定は折り畳み・開閉状態は保存しない
(作成時にプリセットを選んだ chat では済んだ判断の節が開いたままになるため)。
`<summary>` は「プリセット」のみ、ボタンは「プリセットを適用」。

## AC #1 / #3 の確認

実機の編成パネルでの操作をユーザーが確認 (2026-09-20)。API 側は自動テスト済み。

## 追加の検証

`go test -race ./internal/httpapi/ ./internal/service/ ./internal/repository/` を通した
(ロックを 2 本持つ形になったため。レビュアー側の環境では race detector が動かせなかった)。
<!-- SECTION:NOTES:END -->
