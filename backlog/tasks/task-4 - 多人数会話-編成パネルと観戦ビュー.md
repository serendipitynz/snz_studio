...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 UI から参加者編成〜自動進行〜停止まで操作できる (Phase B 完了条件)
- [x] #2 発言に参加者の表示名とモデル名が表示される
- [x] #3 自動進行の停止がターン境界で効くこと (進行中のターンは中断されない) が UI 上で分かる。実行中のターンが表示される (設計 §4.1)
...
## 実機確認 (2026-09-19)
LAN 上の LM Studio 2 台 (192.168.0.103 / 192.168.0.66、いずれも google/gemma-4-e4b) を
参加者 2 人に割り当てて実行し、AC #1 / #2 / #3 をユーザーが画面上で確認した。
編成パネルでの参加者編成・接続確認 (モデル 8 件) からモデル選択、1 ターン進める、
自動進行の開始と停止、発言ごとの表示名とモデル名の表示、停止がターン境界で効く旨の
表示まで動作。これで AC 4 件すべてチェック済み。

なお `wails dev` は、アクティブな Go ツールチェーンが go.mod の指定より新しいと
`internal error: package "math" without types was imported from "snzstudio/internal/vector"`
で落ちる (Wails v2.12.0 の埋め込み x/tools が go1.27 のエクスポートデータを読めない)。
`GOTOOLCHAIN=go1.26.3 wails dev` で回避できる。本タスクとは無関係の環境側の問題なので
別タスクに切り出した。
<!-- SECTION:NOTES:END -->
