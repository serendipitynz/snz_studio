...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 コピー直後、ボタンのアイコンが一定時間チェックマークなど「コピーできた」と分かる見た目に変わり、実アプリ (Wails の WebView) で視認できる
...
## レビュー指摘への対応 (PR #25 round 1)

[P3] AC#1 をチェックしたまま、ノートには「Wails の WebView での視認は未実施」と書いていた、という矛盾の指摘。妥当なので AC#1 のチェックを外した。

AC#1 の文面は「実アプリ (Wails の WebView) で視認できる」であり、実機での視認そのものが達成条件になっている。tsc/build/PNG での目視はアイコンが正しく描かれることの根拠にはなるが、AC が求めている実機確認の代わりにはならない。この「証拠を持たない AC をチェックする」こと自体が TASK-30 を生んだ原因 (TASK-29 の AC#3) なので、ここで繰り返すわけにはいかない。オーナーの実機確認をもって改めてチェックする。
<!-- SECTION:NOTES:END -->
