...
const memoryColumns = `id, project_id, kind, title, content, source_chat_id, created_at, updated_at, source, locked`
...
func scanMemory(s scanner) (model.Memory, error) {
	...
	if err := s.Scan(&m.ID, &m.ProjectID, &m.Kind, &m.Title, &m.Content, &sourceChat, &m.CreatedAt, &m.UpdatedAt, &source, &locked); err != nil {
		return m, err
	}
...
func (r *MemoryRepository) CreateMemory(input CreateMemoryInput) (model.Memory, error) {
	...
	if _, err := tx.Exec(`
		...
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		m.ID, m.ProjectID, m.Kind, m.Title, m.Content, ptrArg(m.SourceChatID), m.Source, boolToInt(m.Locked), m.CreatedAt, m.UpdatedAt); err != nil {
		return model.Memory{}, err
	}
	if _, err := tx.Exec(
