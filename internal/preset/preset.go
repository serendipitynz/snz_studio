...
// Participant is one roster entry of a preset. Only the display name and the
// role prompt are preset data: the endpoint and model are chosen at use time,
// and an empty pair falls back to the workspace endpoint when a turn runs.
...
// participants, which is what makes a chat multi-agent (§2).
