...
アプリに同梱しているプリセット (ディベート・即興劇・TRPG の卓など 25 件) は
...
同梱分はファイル名の先頭の番号の順に一覧に並びます (群ごとの `optgroup` の中でもこの順)。
同梱に足すときは、番号付きのファイル名で `internal/preset/bundled/` に置き、他と重ならない `id` と、
