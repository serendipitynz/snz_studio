...
(同梱の `trpg-table` がこの形です。`design-review` と `cross-era-dialogue` も、提案者・聞き手を進行役にしてこの規則を使います)。

`turnRule` に `weighted` を選ぶと、発言の最後で名指しされた参加者が次に話し、名指しが無ければ長く黙っている参加者に
番が回ります (設計書 §4.6)。進行役を付けると、その参加者は他の参加者より頻繁に番が戻ります。
進行役が毎回相手を選んで問いを向ける編成に向き、同梱の `detective-questioning`・`podcast`・`sengoku-war-council` がこの形です。
進行役の役割プロンプトに「誰に向けた問いかを名指しする」と書いておくと、名指しが検出されやすくなります。
...
- 場を回す参加者 (司会・裁判官・聞き手) を置くときは「自分の意見は言わない」と明記し、先頭に登録する。1 巡ごとにその参加者が要約するので、履歴が切り詰められても文脈が残る。`facilitator_alternating` か `weighted` を使う場合は、この参加者が進行役 (`facilitator: true`) になる。
