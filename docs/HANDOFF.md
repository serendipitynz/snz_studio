...
### Track B — ruri-v3-30m 内蔵 embedding（llama.cpp サイドカー）🚧 スパイク S1/S2 + 追加 A/B/C 全通過 → 実装 Step1-6 完了（2026-05-31, mac arm64 実機・headless 検証済み / GUI・CI・閾値校正は未）

#### 実装状況（2026-05-31・コード完了、未コミット＝ユーザー判断）
Step1-6 を実装し headless 検証は全グリーン（`gofmt`/`go vet`±dev/`go build`(./...+prod)/`GOOS=windows` cross/`go test -race ./...`/`pnpm check:client`/`build:client`）。**manager パイプラインの実機 E2E も合格**（`internal/embed/manager_integration_test.go`: 実 llama-server+GGUF で download-skip→起動→/health→256次元 probe→ready→BaseURL 配信、10秒。env `SNZ_LLAMA_SERVER_BIN`+`SNZ_TEST_RURI_GGUF` 未設定時は skip＝CI 無影響）。
- **Step1 config overlay** `internal/config/config.go`: `Editable.EmbeddingMode`（internal|external）/ `Config` に runtime-only `internalEmbedURL/Model` + `SetInternalEmbedding`/`ClearInternalEmbedding` / `Get()` は internal 時のみ overlay（未準備＝空 model→FTS-only degrade）/ 移行=`applyOverrides` と `New()` で「embeddingMode 欠落＆永続 embeddingModel 非空→external、他→internal」/ `UpdateEditable` は空 mode で現状維持。test 追加（既定/overlay/external非overlay/移行/round-trip）。
- **Step2 新規 `internal/embed/`**（標準ライブラリのみ・新規依存なし）: `modelspec.go`（`RuriV3_30m`: ModelID/FileName/URL=**placeholder**/SHA256=`517044b3…`/SizeBytes=41569120/Dim=256/Query・DocumentPrefix）/ `downloader.go`（sha+size 検証・`.part` resume・atomic rename・idempotent）/ `sidecar.go`（ephemeral port→`--embedding --pooling mean -c 2048 -ngl 0`・/health 待ち・256次元 probe・DYLD_LIBRARY_PATH=bin dir）/ `sidecar_unix.go`・`sidecar_windows.go`（プロセスグループ kill + バイナリパス解決 = mac `Contents/Resources/llama-server` / win 隣 `.exe` / dev `SNZ_LLAMA_SERVER_BIN`）/ `manager.go`（download→supervise、ready/lost コールバック、バックオフ再起動 max5、`Status()`/`BaseURL()`/`Shutdown()`）。test（verify/download/resume/checksum-mismatch/binary 解決/no-binary degrade）。
- **Step3 結線**: `bootstrap.paths` に `ModelsDir`=`dataDir/models` / `app.go` で `embed.NewManager` 構築・`MkdirAll`・internal 時 `EnsureInternalReady(ctx)`（非ブロッキング）・shutdown で `Shutdown()` / `httpapi.NewServer(db,cfg,uploadDir,*embed.Manager)`（テストは nil 可）+ `onEmbeddingReady`（SetInternalEmbedding→RefreshConfiguration→**run-once ガード** `documents/memories.HasEmbeddingsForModel`→go RebuildAll）/ `onEmbeddingLost`（Clear+Refresh）/ `GET /api/embedding/status` / PUT は `embeddingMode` 受理＋internal→EnsureInternalReady・external→Shutdown+Clear、warmup/rebuild は **effective settings（cfg.Get）** 基準。
- **Step4 prefix + 閾値**: `EmbeddingClient.ActivePrefixScheme()`（model==ruri のとき 検索クエリ:/検索文書:、他は no-op）/ query=`retrieval.go` 2箇所で前置（FTS は生）/ document=`embeddingsync.go` の `embedInBatches` で一括前置（doc/memory とも corpus=文書prefix）/ semantic-only fallback 閾値を `semanticFallbackFloor(default, ruri)` で **ruri は 0.9**（external/未設定は 0.55/0.58 維持）。**⚠ ruri 0.9 は S2 の限定データ由来の暫定値＝要 GUI 校正**。
- **Step5 フロント**: `api/client.ts` に `WorkspaceConfiguration.embeddingMode` / `EmbeddingStatus` 型 / `getEmbeddingStatus()` / update payload に embeddingMode。`ProjectListPage` 設定モーダルに Embedding Source トグル（internal/external）＋ external 時のみ endpoint/model フィールド＋internal 時は DL 状態（`embeddingStatusLabel`、downloading/starting 中 2s ポーリング）。
...
#### （旧）スパイク結果（着手前関門は全クリア・蒸し返さない）
**次セッションの開始手順**: (1) この §7 → (2) 計画ファイル `~/.claude/plans/indexed-toasting-nova.md` の §Track B → (3) 既存 Go 層（`internal/service/{embeddingclient,embeddingsync,retrieval}.go`, `internal/config/config.go`, `internal/httpapi/{server,handlers}.go`, `app.go`）。

#### スパイク結果（着手前関門は全クリア・蒸し返さない）
- **ModernBERT は upstream llama.cpp に完全マージ済み（計画前提が好転）**: C++ 推論グラフ `src/models/modern-bert.cpp`（arch `LLM_ARCH_MODERN_BERT="modern-bert"`）も GGUF converter `conversion/bert.py` の `@ModelBase.register("ModernBertModel") class ModernBertModel(BertModel)` も **公式 release `b9437`（2026-05-30）に存在**。Targoyle/ruri-v3-310m-GGUF が言う「パッチ版」は数ヶ月前の話で今は不要。
- **ruri は SentencePiece(Unigram) なので converter に 1 行パッチ（ホスト側・変換時のみ）**: upstream の `ModernBertModel.set_vocab` は英語 ModernBERT 前提で `_set_vocab_gpt2()` を呼ぶため `NotImplementedError: BPE pre-tokenizer was not recognized` で失敗する。**`self._set_vocab_gpt2()` → `self._set_vocab_sentencepiece()` に置換**すれば変換成功（add_bos/eos/sep フラグは維持）。`cl-nagoya/ruri-v3-30m` は実在＝256次元/SentencePiece/ModernBERT-Ja(`sbintuitions/modernbert-ja-30m`)/Apache-2.0/37M・10層/**pooling=mean**/prefix は 1+3（`検索クエリ: `/`検索文書: `/`トピック: `/`""`）。
- **配布 llama-server は公式 release バイナリ（C++ 無改変）で動く**: パッチは Python converter だけ。公式 `b9437` の **macos-arm64 / macos-x64 / win-cpu-x64** で同じ GGUF が `--embedding` で動作（mac x64 は Rosetta で実機起動確認）。**自前クロスビルド不要**。⚠ ただし公式 tar/zip は **dylib/dll に細分化**＝単一バイナリではない。mac 最小バンドル = `llama-server` + 依存 dylib 10種（`libllama-server-impl`/`libllama-common`/`libmtmd`/`libllama`/`libggml{,-cpu,-blas,-metal,-rpc,-base}`、otool -L で確定）= **24MB**。win = `llama-server.exe` + `ggml*.dll`(CPU variant 群) + `llama-common.dll` + `libomp140`。
- **起動フラグ確定**: `llama-server --embedding --pooling mean -c <ctx> --host 127.0.0.1 --port <ephemeral>`（L2 正規化は llama-server デフォルト＝返るベクトルは norm=1）。256次元・非退化・意味妥当を実証（cos(関連)=0.95 ≫ cos(無関係)=0.78〜0.80）。
- **配布 GGUF = q8_0（42MB, f16 は 76MB）**: S2 retrieval 品質は f16 と同等（差 ~0.001）。`modelspec.go` は `cl-nagoya/ruri-v3-30m` の固定 revision から自前変換した q8_0 GGUF を自前ホスト（HF 等）して初回 DL。
- **mac 署名/notarize は実機で Accepted 実証**（証明書 `Developer ID Application: Yoko Otani (9EYB4D9GGQ)` / notary profile `snzstudio`）。**entitlements 確定 = `com.apple.security.cs.allow-jit` + `com.apple.security.cs.allow-unsigned-executable-memory`**（Metal JIT 用、署名済みで Metal 推論も動作）。レシピ: **(1) 不要 exe を全削除**（公式 tar には llama-cli/tokenize/mtmd-debug 等 25 個入っており、`llama-server` 以外を残すと未署名 Mach-O で notarize **Invalid**になる＝B3 への重要追記）→ (2) 全 dylib を `codesign --options runtime --timestamp` → (3) `llama-server` に上記 entitlements 付きで署名 → (4) `ditto -c -k --keepParent` で zip → (5) `notarytool submit --keychain-profile snzstudio --wait`。
- **⚠ S2 で判明した retrieval 調整点（実装必須）**: ruri の cosine は **0.84〜0.88 の狭レンジに集中**（ModernBERT 埋め込みの anisotropy）。相対順位は正しく言い換えクエリで keyword を上回るが、`retrieval.go` の `semanticToUnitRange=(cos+1)/2` だと全部 0.9 台に張り付き、semantic-only fallback の**絶対閾値 0.55/0.58 が緩すぎて常時発火**する。internal(ruri) のとき semantic スコア正規化（候補内 min-max 等）と fallback 閾値を ruri 向けに再調整する（external/未設定時は現挙動を維持）。B1 の「retrieval は無改修」は **prefix と閾値は要改修**に訂正。
