...
**比較のために書いた対案 `addressed_first`**: 呼びかけ先の先頭 1 人が話し、呼びかけが無ければ既存の導出
...
- **複数呼びかけ待ち**: 2 人以上を呼んだ発言について、先に話さなかった側が話すまでに要したターン数の最大値。
...
| --- | --- | --- | --- | --- | --- | --- |
| `weighted`（採用） | 0% | 8 | 1〜3 | 25/25 | **5** | 2 |
| `addressed_first` | 0% | 17 | 1〜6 | 26/26 | **14** | 0 |
| `round_robin` | 0% | 4 | 3〜3 | 13/23 | 3 | 0 |
| `facilitator_alternating` | 0% | 6 | 1〜1 | 11/27 | 5 | 0 |

読み取れたこと:

1. **呼びかけの集中 ×0.5 は、重みへの影響ではなく勝敗の決定として働く。** 呼びかけられた参加者は ×0.5 を
   受けないので重みは最低でも 0.85×0.95 = 0.8075、他の誰かが取りうる最大は 0.5 なので、他の 3 係数が
   何をしようと呼びかけ先が勝つ。呼びかけ先を上げる形に書き換えて上げ幅を下げていくと、境界は
   1/(0.85×0.95) ≈ 1.238 にあり、×1.3 では追従率 17/17、×1.2 以下では 6/19 まで落ちる。
   連続的に効くのではなく、境界の上と下で切り替わる。**Why**: 係数が離散値の固定集合なので、重みの取りうる値も
   数点しかなく、比較が階段関数になる。したがって ×0.5 より強める意味はない。
2. **呼びかけ先が 1 人のあいだ、`weighted` と `addressed_first` は同じことをしている。** 追従率は
   17/17 対 22/22 でどちらも通り、他の観測量の差は進行役の間隔（1〜3 対 1〜1）だけである。
   ここだけを見るなら、4 つの係数を持たない `addressed_first` の方が単純で済む。
3. **呼びかけ先が複数になると差が出る。これが `weighted` を採る理由である。** `addressed_first` は
   呼びかけ先を 1 人しか読めないので `[A, B]` の先頭を採って B を捨てる。B は通常の巡回で戻るしかなく、
   複数呼びかけ待ちが 14 ターン、一巡ターン数が 17 に伸びる。`weighted` では A と B の両方が ×0.5 を
   免れ、残りの係数がどちらから話すかを決めるので、待ちは 5 ターン、一巡は 8 に収まる。
   （`round_robin` の待ち 3 は追従率 13/23 とセットの数字で、呼びかけを追っていない結果なので比較対象にならない。）
4. **同点規則は必須。** 呼びかけのある編成で 60 ターン中 2〜8 回同点になる。名簿順だけで割ると一巡ターン数が
   12 まで伸び、沈黙が長い側を優先すると 7 に収まる。つまり「しばらく黙っていた人に話が回る」性質は、
   直近発言の抑制ではなく**同点規則**が担っている。
5. **進行役が居るときの振る舞いを作っているのは、直近発言の抑制と進行役の免除だけである。**
...
6. **呼びかけが無く進行役も居ない編成では、`weighted` は `round_robin` と 1 発言も違わない。**
   係数を何に変えても発言順は変わらない。`weighted` を選ぶ意味があるのは、呼びかけが起きる編成か、進行役が居る編成である。

#### 4.6.4 決めたこと

- **係数は 4.6.1 の表のとおり**（起票時の原案どおり。読み取り 1 より、呼びかけの集中を強める変更はしない）。
- **同点規則は沈黙が長い側 → 名簿順**（読み取り 4）。
- **人間の介入発言の後は、人間の発言を現ターンの発言者とみなす**（4.6.7）。
- **呼びかけ先は複数を保持する**（読み取り 3）。これが `weighted` を `addressed_first` より優先する理由そのものなので、
  検出も保存も単数に切り詰めない（4.6.5）。

**`addressed_first` を採らない理由**は読み取り 3 に尽きる。呼びかけ先が 1 人のあいだは同じ結果を出し、
係数を持たないぶん単純だが、1 発言が 2 人を呼ぶ場合に一方を捨てるしかなく、捨てられた側の待ちが 14 ターンに伸びる。
「A さんと B さん、どう思う？」は多人数会話でごく普通に起きる発言なので、これは例外扱いにできない。
...
揃えられないのはこの 1 点においてで、名指しの無い介入に限れば両者の結果は近い
（一巡ターン数 7 対 9、追従率 18/20 対 19/20）。`weighted` は §4.5 とは別の規則として並置されるので、
ここが揃わないことは既存の規則の振る舞いを変えない。
...
決め手は、1 発言が 2 人以上を呼ぶ場合に順位規則が一方を捨てるしかなく、捨てられた側の待ちが
14 ターンに伸びること（§4.6.3 読み取り 3）。実装は §4.6.1・§4.6.4〜4.6.7 をそのまま実装すればよい。
