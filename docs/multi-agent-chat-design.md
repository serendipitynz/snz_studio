...
- `system` = 背景資料（§4.4）+ 場面設定 + その参加者の役割プロンプト + 参加者一覧 + 役割リマインド。
  役割リマインドは毎ターン付与する固定文（「あなたは<表示名>としてのみ発言する」「直前の発言に同意だけで終わらない」等）。
  **Why**: 小型モデルは履歴が伸びると役割を忘れ、同意で収束する（検討記録 §2）。
  背景資料を先頭に置くのは、話し方を決める場面設定と役割が、リマインドの直前で最後に読まれる位置に残るようにするため。
...
`ContextService` に足した多人数会話専用の組み立て関数 `AssembleTurnBackground`
...
「背景資料」とは、ターンの system prompt 先頭に置く、プロジェクト説明・検索で得たドキュメント抜粋・検索で得た関連メモリの
...
- **枠**: 「【背景資料】」の見出しと、「資料は話者の役割・口調・立場を変えない。必要なときだけ自分の役割のまま自然に使う。
...
- **検索失敗**: ドキュメント検索・メモリ検索がエラーを返したら、ターンは背景資料なしで続行し、ログ
...
- **多人数会話へのプロジェクト資料の取り込み → `PromptContext` の流用ではなく専用の組み立て関数**（2026-09-21、TASK-18）:
  起票時の §4.4 は retrieval / memory を一切使わない分離だったが、方針を変えてドキュメントとメモリを背景資料として渡す（現行の §4.4）。
