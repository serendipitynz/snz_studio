...
**呼びかけが越えなければならない相手は、直近発言の抑制を免除された進行役である。** 定常状態では編成の
全員が ×0.85 を受けているので、その係数は比較から消える。残るのは免除された進行役の `1.0` と、
呼びかけられた参加者の `0.85 × 加点` で、後者が前者を越える条件は **加点 > 1/0.85 ≈ 1.176** の一本になる。
...
**進行役が居ない編成では、この境界そのものが無い**（越える相手が居ないので、加点の有無だけが効く）。
...
**呼びかけの無い編成**では、`weighted` は `round_robin` とほぼ同じに振る舞う。
...
**呼びかけ先が複数**（呼びかけ 4 割・うち半分が 2 人呼び、呼びかけ先 39 件）:
...
| --- | --- | --- | --- |
| 採用 | 7 / 6 | **11/11** / **12/12** | 1 / 1 |
| 採用・介入時も ×0.2 を課す | 7 / 5 | 11/11 / **0/12** | 1 / 5 |
| 採用・介入後は常に進行役 | 7 / 5 | 11/11 / 11/12 | 1 / 2 |
| 採用・名指しの無い介入は進行役 | 7 / 6 | 11/11 / 12/12 | 1 / 1 |
| 起票時の原案 | 10 / 6 | 11/11 / 12/12 | 1 / 1 |
| `round_robin` | 4 / 4 | 2/11 / 0/12 | 3 / 4 |
...
6. **`addressed_first` は明確に劣る。** 呼びかけ 3 割で一巡ターン数 **30**（採用は 8）、複数呼びかけでは
   窓を持たない変種が未応答 5/39 を残す。呼びかけが常に他を先取りするので、呼ばれない参加者が飢える。
   窓を与えた変種は改善するが（一巡 10）、それでも採用の 8 に届かない。
...
9. **起票時の原案（末尾のみ・他を ×0.5・名簿順あり）との差は小さい。** 呼びかけ先が 1 人のあいだは
   即時追従率も応答待ちも同じで、差は同点の回数（21 対 6）と最大シェア（32% 対 33%）だけである。
   差が出るのは複数呼びかけと名指しの無い介入で、採用案は最大シェア（32% 対 35%）・進行役の間隔
   （1〜5 対 1〜7）・一巡（7 対 10）で勝ち、即時追従では負ける（25/26 対 26/26）。
...
**起票時の原案との差は小さい**（読み取り 9）。呼びかけ先が 1 人のあいだはほぼ同じ結果で、差が出るのは
複数呼びかけと名指しの無い介入である。それでも窓の側を採るのは、応答されない呼びかけが原理的に残らない
（窓のあいだ加点が効き続ける）ことと、係数が 4 つから 3 つに減ることによる。
...
- **名指しの無い介入のときだけ強制する形**: 測定上、採用案と**全指標が一致する**（一巡 7 / 6、
  即時追従 11/11 / 12/12、応答待ち 1 / 1)。進行役の免除によって進行役はすでに応じやすく、
  上書きはその傾向を確定に変えるだけで、この編成では結果を変えなかった。
...
4 人（GM + A/B/C）・呼びかけ 3 割・60 ターン。C だけ係数を下げ、**他はすべて同じ seed で回した**
（基準行だけ別シナリオから引くと、差のどこまでが係数によるものか言えない）:
...
振る舞いが分かれる。測定したシナリオは呼びかけも介入も無いのでその状態に入らない。つまり `weighted` にはすでに参加者ごとの発言頻度が 1 つ、
