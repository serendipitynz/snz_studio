...
| 呼びかけの集中 | 末尾のメッセージが呼びかけた参加者**以外**の全員。呼びかけ先が複数なら、その全員が免れる | ×0.5 |
...
なお同点なら名簿順の先を採る**。一度も発言していない参加者は、発言したことのあるどの参加者よりも沈黙が長いものとして扱う。

**比較のために書いた対案 `addressed_first`**: 呼びかけ先が話し、呼びかけが無ければ既存の導出
（進行役が編成に居れば §4.5 の交互、居なければ `round_robin`）に落ちる、係数を持たない優先順位付きの規則。
呼びかけ先が複数のときの選び方で 2 通り測った — **先頭を採る**形と、**沈黙が長い側を採る**形。
後者が公平な比較相手であり、`weighted` と互角である。不採用の理由は 4.6.4 にある。

#### 4.6.2 シミュレーションと観測量

`weighted` の係数の候補と `addressed_first`、既存 2 規則を [tools/turn-score-sim](../tools/turn-score-sim/main.go) で
60 ターン回して比較した（`go run ./tools/turn-score-sim`）。どの規則も保存済みの発言だけから話者を導く（§2）ので、
...
- **呼びかけ追従率**: 呼びかけのあった次のターンで、呼びかけられた参加者のいずれかが実際に話した割合。
  transcript の末尾にあって、まだ答えが記録されていない呼びかけは分母に入れない（答えを誤ったのではなく、
  シミュレーションの打ち切り位置に当たっただけなので）。
- **複数呼びかけ待ち**: 2 人以上を呼んだ発言について、先に話さなかった側が話すまでに要したターン数の最大値と、
  transcript の終わりまで応答が無かった件数。応答の無かった分を最大値から外すと、誰も答えなかった
  transcript が「待ちが短い」と読めてしまう — 呼びかけ先が待たされていないかを見るための数なので、件数を併記する。
...
呼びかけの強さは、呼びかけ先を上げる形に書き換えて ×2.0（原案と等価）から ×1.0 まで動かした（読み取り 1）。
...
| 4 人（GM+3）・呼びかけなし | `weighted`（採用） | 0% | 5 | 1〜3 | — | 0 |
| | `weighted`（直近発言の抑制なし） | 0% | 4 | 3〜3 | — | 0 |
| | `addressed_first` | 0% | 6 | 1〜1 | — | 0 |
| | `round_robin` | 0% | 4 | 3〜3 | — | 0 |
| | `facilitator_alternating` | 0% | 6 | 1〜1 | — | 0 |
| 4 人（GM+3）・呼びかけ 3 割 | `weighted`（採用） | 0% | 7 | 1〜4 | 17/17 | 2 |
| | `weighted`（同点＝名簿順のみ） | 0% | 12 | 1〜3 | 17/17 | 8 |
| | `weighted`（呼びかけを ×1.1 相当に弱めた形） | 0% | 6 | 1〜3 | 12/20 | 0 |
| | `weighted`（呼びかけの集中なし） | 0% | 5 | 1〜3 | 5/19 | 0 |
| | `addressed_first`（先頭を採る） | 0% | 9 | 1〜3 | 22/22 | 0 |
| | `addressed_first`（沈黙が長い側を採る） | 0% | 9 | 1〜3 | 22/22 | 0 |
| | `round_robin` | 0% | 4 | 3〜3 | 3/19 | 0 |
| | `facilitator_alternating` | 0% | 6 | 1〜1 | 8/19 | 0 |
| 4 人（GM+3）・呼びかけ 3 割 + 名指しの無い介入 | `weighted`（採用） | 0% | 10 | 1〜3 | 18/19 | 4 |
| | `weighted`（採用 + 名指しの無い介入は進行役） | 0% | 7 | 0〜3 | 18/20 | 5 |
| | `addressed_first` | 0% | 9 | 0〜3 | 20/21 | 0 |
| | `round_robin` | 0% | 4 | 3〜3 | 6/20 | 0 |
| 4 人（GM+3）・介入が直前の話者を呼ぶ | `weighted`（介入時も ×0.2 を課す） | 0% | 5 | 1〜3 | **0/12** | 0 |
| | `weighted`（採用） | 0% | 6 | 0〜4 | 12/12 | 1 |
| | `weighted`（介入後は常に進行役） | 0% | 5 | 1〜3 | **0/12** | 0 |
| | `addressed_first` | 0% | 7 | 1〜2 | 12/12 | 0 |
| | `round_robin` | 0% | 4 | 3〜3 | 0/12 | 0 |

呼びかけ先が複数になる場合（4 人（GM+3）、呼びかけ 4 割、うち半分が 2 人呼び）。
「複数呼びかけ待ち」は、2 人以上を呼んだ発言について、先に話さなかった側が話すまでのターン数の最大値と、
transcript の終わりまで応答が無かった件数（呼びかけ先の総数のうち）:

| 規則 | 連続発言率 | 一巡ターン数 | 進行役の間隔 | 呼びかけ追従率 | 複数呼びかけ待ち | 同点 |
| --- | --- | --- | --- | --- | --- | --- |
| `weighted`（採用） | 0% | 8 | 1〜3 | 25/25 | 5（未応答 2/28） | 2 |
| `addressed_first`（沈黙が長い側を採る） | 0% | 10 | 1〜6 | 30/30 | **7（未応答 0/30）** | 0 |
| `addressed_first`（先頭を採る） | 0% | 17 | 1〜6 | 26/26 | 14（未応答 2/26） | 0 |
| `round_robin` | 0% | 4 | 3〜3 | 13/23 | 3 | 0 |
| `facilitator_alternating` | 0% | 6 | 1〜1 | 11/27 | 5（未応答 1/28） | 0 |

読み取れたこと:

1. **呼びかけの集中 ×0.5 では、呼びかけ先が必ず勝つ。これは証明できる。** 呼びかけ先は ×0.5 を受けないので
   重みは最低でも 0.85×0.95 = 0.8075、他の誰かが取りうる最大は 0.5 である。呼びかけ先を上げる形に書き換えると
   （順位としては ×0.5 と等価で、上げ幅 2.0 に当たる）、**最悪の重みの組み合わせでも勝つ十分条件**の境界が
   1/(0.85×0.95) ≈ 1.238 に現れる。ただし**境界の上下で切り替わるのではない**: 境界を下回っても、実際に現れる
   重みの組み合わせ次第で追従は続く。上げ幅 ×1.2 は呼びかけ 3 割・介入なしで 17/17、介入ありで 16/20、
   ×1.1 でそれぞれ 12/20 と 6/19、呼びかけの集中を外すと 5/19 になる。つまり境界より下では**段階的に落ちる**。
2. **呼びかけ先が 1 人のあいだ、`weighted` と `addressed_first` は同じことをしている。** 追従率は
   17/17 対 22/22 でどちらも通り、他の観測量の差は進行役の間隔（1〜4 対 1〜3）と一巡ターン数（7 対 9）だけである。
   ここだけを見るなら、4 つの係数と同点規則を持たない `addressed_first` の方が単純で済む。
3. **呼びかけ先が複数になっても、集合を正しく読む順位規則とは互角である。** `addressed_first` が
   呼びかけ先の**先頭**を採ると、捨てられた側の待ちが 14 ターン・一巡 17 に伸びる。しかしこれは
   その実装の選択であって順位規則の限界ではない。**呼びかけ先の集合から沈黙が長い側を採る**順位規則は、
   係数を 1 つも持たずに追従率 30/30・未応答 0・同点 0 を出す。`weighted` は待ちの最大値（5 対 7）と
   一巡ターン数（8 対 10）で勝つが、28 件のうち 2 件を未応答で終え、同点規則も必要になる。
   **どちらも一方的に優れてはいない。**
4. **どちらの規則も、応答されなかった呼びかけを次の 1 ターンより先へ持ち越さない。** 呼びかけは
   末尾のメッセージからしか読まないので、`[A, B]` のうち A が話した時点で B への呼びかけは視界から外れ、
   B は通常の巡回で戻る。`weighted` の「呼びかけ先の全員が ×0.5 を免れる」は、**その 1 ターンで誰が話すかを
   決める**ものであって、B に順番を保証するものではない。
5. **同点規則は必須。** 呼びかけのある編成で 60 ターン中 2〜8 回同点になる。名簿順だけで割ると一巡ターン数が
   12 まで伸び、沈黙が長い側を優先すると 7 に収まる。つまり「しばらく黙っていた人に話が回る」性質は、
   直近発言の抑制ではなく**同点規則**が担っている。
6. **進行役が居るときの振る舞いを作っているのは、直近発言の抑制と進行役の免除だけである。**
   ×0.85 を外すか進行役を免除から外すと、発言順は `round_robin` と完全に一致する（間隔 3〜3）。
   免除を効かせると間隔は 1〜4 になるが、これは `facilitator_alternating` の確定的な 1〜1 にも
   `round_robin` の規則的な 3〜3 にもならない中間で、起票時に予想した「進行役が 1 ターンおきに挟まる」は再現しない。
7. **呼びかけが無く進行役も居ない編成では、`weighted` は `round_robin` と 1 発言も違わない。**
   係数を何に変えても発言順は変わらない。`weighted` を選ぶ意味があるのは、呼びかけが起きる編成か、進行役が居る編成である。

#### 4.6.4 決めたこと

**`weighted` を採る**（2026-09-22、ユーザー判断）。

- **係数は 4.6.1 の表のとおり**（起票時の原案どおり）。呼びかけの集中は ×0.5 のまま、強めも弱めもしない。
  強めても結果は 1 発言も変わらず（読み取り 1）、弱めると呼びかけが通らなくなるためである。
- **同点規則は沈黙が長い側 → 名簿順**（読み取り 5）。
- **人間の介入発言の後は、人間の発言を現ターンの発言者とみなす**（4.6.7）。進行役を強制する上書きは入れない。
- **呼びかけ先は複数を保持する**（4.6.5）。

**採る理由は測定上の優位ではなく、呼びかけを他の圧力と混ぜられるという表現力にある。**
読み取り 2・3 のとおり、集合を正しく読む順位規則（`addressed_first`（沈黙が長い側））は、呼びかけ先が
1 人でも複数でも `weighted` と互角で、係数も同点規則も持たないぶん単純である。トレードオフは
「待ちの最大値と一巡ターン数では `weighted`、未応答件数と同点の無さでは順位規則」で、決着していない。

`weighted` にしかできないのは、**呼びかけの強さを他の係数と同じ土俵で調整すること**である。
呼びかけの集中を弱めていくと追従率は段階的に下がり、「呼ばれた人がだいたい答えるが、長く黙っていた人が
割り込むこともある」設定を作れる。呼びかけ 3 割・介入なしの条件で、上げ幅 ×2.0（原案と等価）・×1.3・×1.2 が
17/17、×1.1 が 12/20、呼びかけの集中なしが 5/19 である（読み取り 1）。
順位規則には呼びかけが常に最優先という一点しかなく、この調整幅が無い。**既定は原案の係数**（呼びかけを
必ず通す設定）とし、調整は将来の余地として残す。

**起票時にこの節が書いていた「順位規則は呼びかけ先の一方を捨てるしかない」は誤り**だった。
それは比較のために書いた実装が呼びかけ先の先頭を採っていたためで（待ち 14 ターン・一巡 17）、
集合から沈黙が長い側を採る順位規則は待ち 7・未応答 0 を出す。読み取り 3 がその測定である。

**どちらの規則も、応答されなかった呼びかけを持ち越さない**（読み取り 4）。持ち越すには呼びかけの待ち行列が
必要で、§2 の「サーバー側に進行状態を持たない」と両立させるには「まだ応答されていない呼びかけ」を
保存済みの発言から遡って導く規則が要る。この spike では設計していない。必要になったら別タスクで扱う。
...
再起動の前後で次の話者が変わりうる（§2）。判定のための追加の LLM 呼び出しは行わない
...
呼びかけ先は ×0.5 を免れても `0.2 × 0.85 × 0.95 = 0.1615` にしかならず、他の誰かの最大 0.5 に負ける。
測定では追従率 **0/12**、つまり一度も届かない。人間の発言を現ターンの発言者とみなすと `0.85 × 0.95 = 0.8075`
になり **12/12** に変わる。意味の上でも、人間が間に挟まったのだから、その participant は続けて話してはいない。
...
名指しの無い介入での差は一巡ターン数 10 対 7・進行役の間隔 1〜3 対 0〜3 で、上書きのある方がやや締まる。
...
測定上は、呼びかけ先の集合から沈黙が長い側を採る順位規則と `weighted` は互角で
（§4.6.3 読み取り 2・3）、採った理由は呼びかけの強さを他の係数と同じ土俵で調整できる表現力にある
（§4.6.4）。実装は §4.6.1・§4.6.4〜4.6.7 をそのまま実装すればよい。
