...
既存の 3 規則を置き換えずに第 4 のターン進行ルール `weighted` として表現する設計。**実装はまだ無い**
（起票は 4.6.9）。決めた係数・規則と、それを支える測定をここに置く。実装時に §2・§3・§5・§6 へ反映する。
...
- **TASK-34**: `weighted` と呼びかけの検出・保存（4.6.1・4.6.4・4.6.5・4.6.7）。TASK-33 に依存する。
...
| 将来 | TRPG 対応（chat 単位の JSON 状態の保持と注入・コードによるダイス・構造化出力での判定宣言）、発言本文の呼びかけを重みに織り込む `turn_rule` の追加値（`weighted`。設計は §4.6 で済んでおり、その上位集合にあたる「進行役がモデル出力で次の話者を選ぶ」規則は未設計。決定的に交互に挟む `facilitator_alternating`（§4.5）は実装済み）、retrieval 統合、生成中断のための呼び出し元 `context` の伝播（§4.1） | — |
...
着手時に判断する論点は §8.2 にある。ただし `weighted` は TASK-28（spike）で設計まで済ませてあり、
...
**発言本文の呼びかけから次の話者を導く規則については、着手時に判断する論点は残っていない**
...
実装は §4.6.1・§4.6.4〜4.6.7 をそのまま実装すればよい。
