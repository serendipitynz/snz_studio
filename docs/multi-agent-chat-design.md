...
| `addressed_first`（窓・未応答・沈黙が長い側） | 47% | 12 | 21/21 | 1（21 件・未応答 0） | 0 |
...
6. **窓を与えた順位規則とは互角で、傾向が違う。** `addressed_first`（窓・未応答・沈黙が長い側）は
   ...
   `weighted` は一巡ターン数（7 対 10、5 対 12）と未応答（0 対 1）と最大シェア（33% 対 35%、37% 対 47%）で勝つ。
...
（4.6.3 の 2 番目の表）、上書きで得られるものが無くなった。名指しのある介入では即時追従率が 9/12 まで
落ちるので、上書きは害の側に回る。
