...
視界から消える。窓と ×1.2 に替えると、呼びかけは他の係数と競合しながら、応答されるまで効き続ける。

**加点の値の下限は、進行役の免除に勝てるかどうかで決まる。** 定常状態では編成の全員が直近発言の抑制を
受けているが、**進行役だけが免除されている**ので、呼びかけられた参加者が越えなければならない相手は進行役である。

- 呼びかけられた参加者が名簿順の次でないとき、その重みは `0.85 × 0.95 × 加点 = 0.8075 × 加点`。
- 進行役が名簿順の次でないときの重みは `0.95`。これを越えるには **加点 > 0.95 / 0.8075 = 1/0.85 ≈ 1.176**。
- 進行役が名簿順の次のときの重みは `1.0`。これを越えるには **加点 > 1 / 0.8075 = 1/(0.85 × 0.95) ≈ 1.238**。
  こちらは「どの重みの組み合わせでも呼びかけ先が勝つ」十分条件である。

**この 2 つの数は保証の境界であって、挙動が切り替わる点ではない。** 下限を下回っても、その場の重みの
組み合わせ次第で呼びかけ先が勝つことはある（進行役が直近に発言していて免除が効かない場合など）。
逆に上限を超えても窓は働き続ける（応答した参加者は加点を失い、未応答の参加者は保つので、
末尾のメッセージのみを読む形とは違う結果になる）。**そのうえで採用値の ×1.2 は、この帯の中にあるだけでなく、
測定で選んだ値である**（4.6.3 読み取り 1: 進行役が居る編成で ×1.1 は呼びかけをほぼ通さず、
×1.3 以上は未応答の呼びかけを作る。×1.2 だけが未応答 0 になる）。
**進行役が居ない編成ではこの帯が成立しない**ので、加点の下限も違う（3 人・進行役なしでは ×1.1 でも
即時追従率 13/13）。
...
**比較のために書いた対案 `addressed_first`**: 呼びかけ先が話し、呼びかけが無ければ既存の導出
（進行役が編成に居れば §4.5 の交互、居なければ `round_robin`）に落ちる、係数を持たない優先順位付きの規則。
呼びかけ先の選び方と窓の有無で 3 変種を測った — 先頭を採る形、集合から沈黙が長い側を採る形、
そして**窓のうちで未応答のものから沈黙が長い側を採る形**。最後のものが公平な比較相手である。
不採用の理由は 4.6.4 にある。
...
**変種は一度に 1 つだけ変える。** 採用案を基準に、加点の値・窓の大きさ・応答で消えるか・同点規則・
人間の介入の扱いをそれぞれ単独で振った行を持つ。以前の一覧は加点と応答方針を同時に振っていたため、
差がどちらから来たのか言えなかった。
...
- **応答待ち**: 呼びかけられた参加者が実際に話すまでに要したターン数の最大値と、transcript の終わりまで
  応答が無かった件数。**窓つきの形は遅れて応答することを許すので、即時追従率だけでは評価できない。**
  即時追従率が下がっても応答待ちが短く未応答が 0 なら、呼びかけは 1〜数ターン遅れて通っている。
- **複数呼びかけ待ち**: 2 人以上を呼んだ発言についての応答待ち。
- **同点**: 60 ターンのうち、重みの最大値が 2 人以上で並んだターン数。

#### 4.6.3 測定結果

呼びかけが無い編成（3 人・4 人、進行役なし）では、**`weighted` は係数を何に変えても `round_robin` と
1 発言も違わない**。進行役が居る編成（4 人）では、直近発言の抑制と進行役の免除だけが違いを作る:
採用案は間隔 1〜3・一巡 5・最大シェア 40%、どちらかを外すと `round_robin` と完全一致して間隔 3〜3・一巡 4。
以下は呼びかけが起きる編成の比較で、断りのない限り 4 人（GM + A/B/C）・60 ターン。

**呼びかけ先が 1 人のとき**（呼びかけ 3 割）:

| 規則 | 最大シェア | 一巡 | 進行役の間隔 | 即時追従 | 応答待ち | 同点 |
| --- | --- | --- | --- | --- | --- | --- |
| `weighted`（採用） | 37% | 7 | 1〜4 | 17/17 | 1（17 件・未応答 0） | 2 |
| `weighted`（加点 ×1.1） | 40% | 6 | 1〜3 | 5/18 | 4（18 件・未応答 0） | 0 |
| `weighted`（加点 ×1.3 / ×2.0、結果は一致） | 37% | 7 | 1〜4 | 17/17 | 1（17 件・未応答 0） | 2 |
| `weighted`（加点なし） | 40% | 5 | 1〜3 | 5/19 | 4（未応答 1/19） | 0 |
| `weighted`（応答でも残る） | 40% | 9 | 1〜3 | 17/21 | 3（21 件・未応答 0） | 3 |
| `weighted`（同点＝名簿順のみ） | 37% | 12 | 1〜3 | 17/17 | 1 | 8 |
| `weighted`（起票時の原案。末尾のみ・他を ×0.5） | 37% | 7 | 1〜4 | 17/17 | 1（17 件・未応答 0） | 2 |
| `addressed_first`（窓・未応答・沈黙が長い側） | 43% | 9 | 1〜3 | 22/22 | 1（22 件・未応答 0） | 0 |
| `round_robin` | 25% | 4 | 3〜3 | 3/19 | 3（未応答 1/20） | 0 |
| `facilitator_alternating` | 50% | 6 | 1〜1 | 8/19 | 5（未応答 1/20） | 0 |

**呼びかけ先が 1 人 + 名指しの無い人間の介入**（呼びかけ 3 割・7 ターンごとに介入）:

| 規則 | 最大シェア | 一巡 | 即時追従 | 応答待ち | 同点 |
| --- | --- | --- | --- | --- | --- |
| `weighted`（採用） | 37% | **5** | 9/20 | 4（20 件・**未応答 0**） | 2 |
| `weighted`（加点 ×1.3 / ×2.0） | 37% | 8 | 19/19 | 1（19 件・未応答 0） | 4 |
| `weighted`（加点 ×1.1） | 40% | 5 | 4/19 | 4（未応答 1/19） | 1 |
| `weighted`（名指しの無い介入は進行役） | 40% | 8 | 16/20 | 7（**未応答 1**/21） | 4 |
| `weighted`（起票時の原案） | 37% | 10 | 18/19 | 2（**未応答 1**/20） | 4 |
| `addressed_first`（窓・未応答・沈黙が長い側） | 47% | 12 | 21/21 | 1（**未応答 1**/22） | 0 |
| `round_robin` | 25% | 4 | 6/20 | 3（未応答 2/21） | 0 |

**呼びかけ先が複数になるとき**（呼びかけ 4 割、うち半分が 2 人呼び）:

| 規則 | 最大シェア | 一巡 | 即時追従 | 応答待ち | 複数呼びかけ待ち | 同点 |
| --- | --- | --- | --- | --- | --- | --- |
| `weighted`（採用） | **33%** | **7** | 17/27 | **4（45 件・未応答 0）** | **4（未応答 0）** | **0** |
| `weighted`（加点 ×1.3 / ×2.0） | 30% | 9 | 23/26 | 4（未応答 2/44） | 4（未応答 2/34） | 2 |
| `weighted`（加点 ×1.1 / 加点なし、結果は一致） | 40% | 5 | 18/26 | 4（未応答 1/41） | 4（未応答 1/30） | 0 |
| `weighted`（応答でも残る） | 37% | 9 | 18/20 | 5（36 件・未応答 0） | 5（未応答 0） | 2 |
| `weighted`（起票時の原案） | 35% | 8 | 25/25 | 5（未応答 2/40） | 5（未応答 2/28） | 2 |
| `addressed_first`（窓・未応答・沈黙が長い側） | 35% | 10 | 27/29 | 3（未応答 1/45） | 3（未応答 1） | 0 |
| `addressed_first`（沈黙が長い側・窓なし） | 37% | 10 | 30/30 | 7（46 件・未応答 0） | 7（未応答 0） | 0 |
| `addressed_first`（先頭・窓なし） | 42% | 17 | 26/26 | 14（未応答 2/40） | 14（未応答 2/26） | 0 |
| `round_robin` | 25% | 4 | 13/23 | 3（未応答 1/38） | 3 | 0 |

**人間の介入が直前の話者を呼ぶとき**（5 ターンごと）と、**編成の全員を呼ぶとき**（6 ターンごと）:

| 規則 | 条件 | 一巡 | 即時追従 | 応答待ち |
| --- | --- | --- | --- | --- |
| `weighted`（採用） | 直前の話者を呼ぶ | 6 | 11/12 | 2（12 件・未応答 0） |
| `weighted`（介入時も ×0.2 を課す） | 同上 | 6 | **0/12** | 2（未応答 1/12） |
| `weighted`（介入後は常に進行役） | 同上 | 6 | 9/12 | 2（12 件・未応答 0） |
| `round_robin` | 同上 | 4 | 0/12 | 4（未応答 1/12） |
| `weighted`（採用） | 編成の全員を呼ぶ | 5 | 10/10 | 5（未応答 3/40） |
| `weighted`（加点なし） | 同上 | 5 | 10/10 | 5（未応答 3/40） |

読み取れたこと:

1. **加点 ×1.2 は帯の中にあるだけでなく、測定で選んだ値である。** 進行役が居る編成では、×1.1 は
   呼びかけをほぼ通さず（即時追従 5/18。加点なしの 5/19 とほぼ同じ）、×1.3 以上は複数呼びかけで
   未応答を 2 件作る。**×1.2 だけが複数呼びかけで未応答 0 になる。** 帯の下限 1.176 と上限 1.238 は
   どちらも「進行役の免除に勝てるか」から出る保証の境界で（4.6.1）、挙動が切り替わる点ではない:
   下限を下回っても勝つ場合があり、上限を超えても窓は働き続ける。
   **進行役が居ない編成ではこの帯が効かない**（3 人・進行役なしでは ×1.1 でも即時追従 13/13）。
2. **窓を入れると、応答されない呼びかけが無くなる。** 起票時の原案（末尾のみ）と比べて、複数呼びかけで
   未応答 2/40 → **0/45**、名指しの無い介入があるシナリオで 1/20 → **0/20**。原案は呼びかけを末尾の
   メッセージ 1 つからしか読まないので、`[A, B]` の A が話した瞬間に B への呼びかけが視界から消えていた。
3. **即時追従率は下がるが、それは「即時」を測っているためである。** 複数呼びかけで 25/25 → 17/27、
   名指しの無い介入ありで 18/19 → 9/20 に落ちる一方、応答待ちの最大値は 4 ターンで未応答は 0 になる。
   呼びかけは失われたのではなく、1〜数ターン遅れて通っている。
4. **窓は一巡ターン数と発言の偏りも改善する。** 名指しの無い介入があるシナリオで一巡 10 → **5**、
   複数呼びかけで 8 → **7**・最大シェア 35% → **33%**。呼びかけが即時に勝ち切らなくなった分、
   他の係数（沈黙・名簿順）が働く余地が戻る。
5. **応答で消える形が、残る形より測定上ほぼ全面的に優る。** 複数呼びかけで一巡 7 対 9・
   最大シェア 33% 対 37%・同点 0 対 2。ただし**残る形でも会話は破綻しない**: 最大シェアを取るのは
   応答した参加者ではなく進行役で（37%）、それは起票時の原案（35%）でも同じである。
   連続発言の抑制 ×0.2 が直後の再登板を止め、直近発言の抑制 ×0.85 が加点の 1.2 を 1.02 まで削るためである。
6. **窓を与えた順位規則とは互角で、傾向が違う。** `addressed_first`（窓・未応答・沈黙が長い側）は
   即時追従率で勝ち（複数呼びかけで 27/29 対 17/27、名指しの無い介入ありで 21/21 対 9/20）、
   `weighted` は一巡ターン数（7 対 10、5 対 12）と未応答（複数呼びかけで 0/45 対 1/45、
   名指しの無い介入ありで 0/20 対 1/22）と最大シェア（33% 対 35%、37% 対 47%）で勝つ。
   順位規則では呼びかけが常に他を先取りするため巡回が伸び、`weighted` では呼びかけが他の圧力と
   競合するため巡回が保たれる。
7. **同点規則は依然として必須。** 窓を入れると複数呼びかけのシナリオでは同点が 0 になるが、
   呼びかけ 3 割と介入ありのシナリオでは 2 回残る。名簿順だけで割ると一巡ターン数が 12 まで伸びる。
8. **人間の介入発言の後に ×0.2 を課し続けると、名指しの介入が次のターンで一度も届かない**（即時追従 0/12）。
   窓があるので 2 ターン以内には届くが 12 件中 1 件は届かないままで、人間の発言を現ターンの発言者と
   みなす形（11/12・応答待ち 2・未応答 0）より劣る（4.6.7）。
9. **編成の全員を呼ぶ介入では、加点は結果をほとんど変えない**（一巡・即時追従・応答待ちが加点なしと同値、
   最大シェアが 38% 対 40%）。全員が加点を受けるので相対的な差が付かないためである。
   ただし**完全に同じではない**ので、検出側で「全員一致は呼びかけ無し」と捨てる扱いは採らない（4.6.5）。

#### 4.6.4 決めたこと

**`weighted` を採る**（2026-09-22、ユーザー判断）。係数は 4.6.1 の表のとおり。

- **呼びかけは窓つきの加点で表す**（窓 = 編成の人数、×1.2、応答で消える）。起票時の原案「呼びかけ先以外を
  ×0.5」から替えた。**Why**: 原案は末尾のメッセージ 1 つしか読まないので、応答されない呼びかけが残る
  （読み取り 2）。窓つきの ×1.2 は、未応答を 0 にしつつ一巡ターン数と発言の偏りも改善する（読み取り 4）。
- **検出側で「編成の全員への呼びかけ」を捨てる特別扱いは置かない**（読み取り 9、4.6.5）。
- **加点は応答で消える**（読み取り 5）。残る形でも破綻はしないが、測定上ほぼ全面的に劣る。
- **同点規則は沈黙が長い側 → 名簿順**（読み取り 7）。
- **人間の介入発言の後は、人間の発言を現ターンの発言者とみなす**（4.6.7）。進行役を強制する上書きは入れない。
- **呼びかけ先は複数を保持する**（4.6.5）。

**採る理由は、呼びかけを他の圧力と競合させられることにある。**
窓を与えた `addressed_first` とは互角で、傾向が違う（読み取り 6）。順位規則は呼びかけを常に先取りするので
即時追従率で勝つが、そのぶん巡回が伸びる（一巡 10 対 7）。`weighted` は呼びかけを 4 つの係数の 1 つとして
扱うので、呼びかけが 1〜数ターン遅れて通る代わりに巡回と発言の偏りが保たれる。
**どちらが「自然」かは測定では決まらない。** 会話の見え方を選ぶ判断であり、ここでは後者を採った。

**加点の値は測定で選んだものとして扱う**（読み取り 1）。`1/0.85 ≈ 1.176` と `1/(0.85 × 0.95) ≈ 1.238` は
どちらも「進行役の免除に勝てるか」から出る保証の境界で（4.6.1）、挙動が切り替わる点ではない。
採用値 ×1.2 を選んだのは測定の結果である: 進行役が居る編成で ×1.1 は呼びかけをほぼ通さず（即時追従 5/18）、
×1.3 以上は複数呼びかけで未応答を 2 件作り、×1.2 だけが未応答 0 になる。
**値を動かしたらこの節の測定をやり直す。** 進行役が居ない編成では下限が効かないので、
編成の形によって最適値が違いうることも含めて測り直す必要がある。

**起票時にこの節が書いていた 2 つの主張は誤りだった**ので取り消してある。
(1)「順位規則は呼びかけ先の一方を捨てるしかない」— 比較用の実装が集合の先頭を採っていたためで、
集合から選ぶ順位規則は成り立つ（読み取り 6）。
(2)「呼びかけはスイッチとして切り替わる」— 1.238 は十分条件の境界で、下回ると段階的に落ちる（読み取り 1）。
...
- **(b) 名前の照合（(a) が空振りしたときだけ）**: 発言の最終文に現れた編成の表示名のうち、自分自身を除いた
  ものすべてを呼びかけ先とする。(a) と違い上限を置かないのは、複数呼びかけを表現できることが `weighted` の
  利点だからである（読み取り 3）。ただし**最終文に限り、表示名は 2 文字以上**とする。
  ...
  未応答の参加者は保つので、捨てた場合と残した場合で発言順が変わる（4.6.3 読み取り 9）。
  残すほうが「みなさん、どうですか」に対して編成が一巡する挙動になるので、特別扱いを置かない。
...
`weighted` を足すと複製は 4 係数と同点規則の分まで増え、検出結果（`addressed_participant_ids`）をフロントへ
...
呼びかけ先は ×1.2 の加点を受けても `0.2 × 0.85 × 0.95 × 1.2 ≈ 0.194` にしかならず、他の誰かの 0.85 以上に負ける。
測定では即時追従率 **0/12**、つまり次のターンでは一度も届かない。呼びかけの窓があるので 2 ターン以内には
届くが、12 件のうち 1 件は届かないままである。人間の発言を現ターンの発言者とみなすと
`0.85 × 0.95 × 1.2 ≈ 0.969` になり **11/12**（応答待ち 2 ターン・**未応答 0**）に変わる。
意味の上でも、人間が間に挟まったのだから、その participant は続けて話してはいない。

**名指しの介入でも、即時に応答するとは限らない**（11/12 で、1 件は 1 ターン遅れる）。進行役が名簿順の次に
当たっていると `1.0` になり、呼びかけられた参加者の `0.969` を上回るためである。呼びかけは窓のあいだ
残るので次のターンで通る。即時に応答させたいなら呼びかけを最優先の規則にする必要があり、それは
`weighted` を採らないという判断になる（4.6.4）。

名指しの有無によらず介入後は進行役を強制する形も測ったが、即時追従率 **9/12** で採用案の 11/12 に劣る。
TASK-21 が `facilitator_alternating` について決めた「人間の介入発言の次は進行役」（§4.5 手順 2）と
揃えられないのはこの 1 点においてで、`weighted` は §4.5 とは別の規則として
並置されるので、ここが揃わないことは既存の規則の振る舞いを変えない。

**名指しの無い介入のときだけ進行役を強制する上書きも測ったが、入れない。** これは損得のある取引で、
「何も得られない」からではない。名指しの無い介入があるシナリオ（4.6.3 の 2 番目の表）で、上書きは
即時追従率を **9/20 → 16/20** に上げる代わりに、一巡ターン数を **5 → 8**、未応答を **0/20 → 1/21**、
同点を **2 → 4** にする。名指しのある介入は上書きの対象外なので、そちらの即時追従率は 11/12 のまま動かない。

**入れない理由**: 失うもの（未応答 0 と短い一巡）が、窓つきの加点を採った理由そのものである
（4.6.4、読み取り 2・4）。介入の直後に進行役が応じることを確定させたいなら、`facilitator_alternating`
を選べば規則として得られる（§4.5 手順 2）。`weighted` の側に上書きを足すと、「重みで決める」一点で
説明できる規則が、条件付きの例外を 1 つ抱えた規則になる。

#### 4.6.8 §0・§7 の非ゴールとの関係

§0 と §7 が将来へ置いている**進行役による指名**は、「**進行役が**モデル出力で次の話者を選ぶ」ことを指す。
`weighted` はモデルの出力を読む点で同種だが、読む対象を進行役に限らず**どの参加者の発言でも**、
さらに人間の介入発言でも同じに扱い、しかも読んだ結果は次の話者を決める 4 つの係数のうちの 1 つにすぎない。
...
| 呼びかけの効き方 | 応答者リストに入れる（他の参加者も抽選で入りうる） | ×1.2 の加点（他の 3 係数と競合する） |
...
定数係数では**構造的に勝てない参加者が生まれる**。編成 GM/A/B/C で C だけ係数を下げて 60 ターン回すと
（呼びかけ 3 割）:

| C の係数 | 一巡ターン数 | 応答待ち | 最大シェア | 同点 |
| --- | --- | --- | --- | --- |
| 1.0（係数なし = 採用案） | 7 | 1（17 件・未応答 0） | 37% | 2 |
| 0.8 | 24 | 3（12 件・未応答 0） | 42% | 4 |
| 0.5 | **一巡せず** | 1（**未応答 5/11**） | 42% | 0 |
| 0.2 | **一巡せず** | 1（**未応答 8/18**） | 42% | 1 |

**0.5 の時点で C は 60 ターンのうち一巡に加わらなくなる。** 「少し控えめ」と設定したつもりの値で
その参加者が会話から落ちる。「控えめ」を設定した利用者が得るのは「無言」であり、
同じ名前の設定が先行実装と違う意味になる。
...
**ただし、進行役の免除は `talkativeness` の特殊ケースであることが測定で分かった。** 免除を外して
進行役に係数 1.15 を与えると、4 人（GM + 3）の編成で採用案と全指標が一致する（進行役の間隔 1〜3、
一巡ターン数 5、最大発言シェア 40%）。つまり `weighted` にはすでに参加者ごとの発言頻度が 1 つ、
コードに焼き込まれている。将来 `talkativeness` を入れるなら、免除をデータ側へ移して特殊ケースを
無くせる（今やっても測定上は何も変わらないので、TASK-34 の範囲外とする）。
