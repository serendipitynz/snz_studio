...
ステータス: **DRAFT / 実装前設計**（2026-09-09 起票） / 背景: [multi-agent-chat-notes.md](./multi-agent-chat-notes.md)
...
> この文書は確定仕様ではない。§8 の「未決」を潰しながら更新する。
...
- **プリセット**とは、参加者一式・ターン進行ルール・場面設定の雛形をまとめた、多人数会話の新規作成時に適用できるデータを指す。
...
- 履歴は直近 30 発言に制限する（定数 `turnHistoryLimit`）。要約による圧縮は行わない（§8 で解消、理由はそこに書く）。
...
**プリセットの適用**とは、プリセットの `turnRule` / `scenePrompt` を持つ chat を作り、`participants[]` を登録順（= round_robin の巡回順）に
参加者として作ることを指す。chat の title が空ならプリセットの `title` を使う。参加者の接続先・モデルは空で作られ、
ターン実行時はワークスペース既定のエンドポイントに落ちる（参加者ごとに変えるのは編成パネル）。
適用後の chat はプリセットとの結びつきを持たない（以後の編集はすべて編成パネル）。参加者の作成が途中で失敗したら chat ごと削除して返す
...
- **プリセット**: 新規作成フォームで `kind = multi_agent` を選ぶとプリセット選択が出る。同梱プリセットは群（`discussion` 議論・検討 /
  ...
  保存形式は §8 で確定: 同梱分は `internal/preset/bundled/*.json` を `go:embed`、追加分は `presets/multi-agent/*.json`（形式の説明は同ディレクトリの README）。
...
- ~~履歴圧縮への summary サービス流用の方式~~ → **解消（2026-09-19、見送り）**: 直近 30 発言の切り詰めのみとし、要約は入れない。
...
- ~~プリセットの保存形式~~ → **解消（2026-09-19）**: 同梱 JSON（`internal/preset/bundled/*.json` を `go:embed`）+ JSON ファイルからのインライン適用（§5 `preset`）。
...
- AGENTS.md「Core product shape」への多人数会話の追記文面（実装着手時に反映する）。
