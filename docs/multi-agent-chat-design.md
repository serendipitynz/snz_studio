...
その値では他の 3 係数が何をしようと呼びかけ先が必ず勝ち（4.6.3 読み取り 1）、しかも呼びかけは末尾の
メッセージ 1 つからしか読めないので、`[A, B]` の A が話した瞬間に B への呼びかけが視界から消える。
窓と ×1.2 に替えると、呼びかけは他の係数と競合しながら、応答されるまで効き続ける。

**×1.2 という値には範囲としての意味がある。**

- `1/0.85 ≈ 1.176` 以下では、直近発言の抑制が加点を打ち消してしまう。呼びかけられた参加者は一度発言すると
  ×0.85 を受けるので、加点がこれ以下だと呼びかけが事実上届かない（×1.1 の実測で即時追従率 7/19）。
- `1/(0.85 × 0.95) ≈ 1.238` 以上では、最悪の重みの組み合わせでも呼びかけ先が勝つ。呼びかけが即時に
  勝ち切るので窓が働く余地がなくなり、原案の ×0.5 と同じ振る舞いに戻る（×1.5 と ×2.0 の実測が一致）。
- **×1.2 はこの 2 つの境界の間にある。** 呼びかけは通るが必ず勝つわけではなく、窓のあいだ効き続ける。
...
同点規則は 2 つ比べた（名簿順の先を採るもの、沈黙が長い側を採り、なお同点なら名簿順に落ちるもの）。
人間の介入発言の後の扱いは 3 つ比べた: 直近の参加者発言者に介入後も ×0.2 を課すもの、人間の発言を
現ターンの発言者とみなすもの、名指しの無い介入では進行役を強制するもの（4.6.7）。
呼びかけの係数は、末尾のメッセージのみの形で ×2.0（原案と等価）から ×1.0 まで、
窓つきの形で ×1.1 / ×1.2 / ×1.5 / ×2.0、および応答で消える／残るの両方を測った。

#### 4.6.3 測定結果

呼びかけが無い編成（3 人・4 人、進行役あり／なし）では、**`weighted` は係数を何に変えても
`round_robin` と 1 発言も違わない**。進行役が居る編成では、直近発言の抑制と進行役の免除だけが
違いを作る（間隔 1〜4、一巡 5。どちらかを外すと `round_robin` と完全一致して間隔 3〜3、一巡 4）。
以下は呼びかけが起きる編成の比較で、いずれも 4 人（GM + A/B/C）・60 ターン。

**呼びかけ先が 1 人のとき**（呼びかけ 3 割）:

| 規則 | 最大シェア | 一巡 | 進行役の間隔 | 即時追従 | 応答待ち | 同点 |
| --- | --- | --- | --- | --- | --- | --- |
| `weighted`（採用。窓 4・×1.2・応答で消える） | 37% | 7 | 1〜4 | 17/17 | 1（17 件・未応答 0） | 2 |
| `weighted`（窓なし・呼びかけ先以外 ×0.5） | 37% | 7 | 1〜4 | 17/17 | 1（17 件・未応答 0） | 2 |
| `weighted`（窓 4・×1.1） | 42% | 6 | 1〜3 | 7/19 | — | 0 |
| `weighted`（同点＝名簿順のみ） | 37% | 12 | 1〜3 | 17/17 | 1 | 8 |
| `addressed_first`（窓 4・未応答・沈黙が長い側） | 43% | 9 | 1〜3 | 22/22 | 1（22 件・未応答 0） | 0 |
| `round_robin` | 25% | 4 | 3〜3 | 3/19 | — | 0 |
| `facilitator_alternating` | — | 6 | 1〜1 | 8/19 | — | 0 |
...
| `weighted`（窓なし） | 37% | 10 | 18/19 | 2（**未応答 1**/20） | 4 |
| `addressed_first`（窓 4・未応答・沈黙が長い側） | 47% | 9 | 20/21 | 4（未応答 1/22） | 0 |
| `round_robin` | 25% | 4 | 6/20 | 3（未応答 1/38） | 0 |
...
| `weighted`（窓 4・×1.2・応答でも残る） | 37% | 9 | 18/20 | 5（36 件・未応答 0） | 5（未応答 0） | 2 |
| `weighted`（窓 4・×2.0） | 30% | 9 | 23/26 | 4（未応答 2/44） | 4（未応答 2/34） | 2 |
| `weighted`（窓なし・原案の ×0.5） | 35% | 8 | 25/25 | 5（未応答 2/40） | 5（未応答 2/28） | 2 |
| `addressed_first`（窓 4・未応答・沈黙が長い側） | 35% | 10 | 27/29 | 3（未応答 1/45） | 3（未応答 1） | 0 |
...
**人間の介入が直前の話者を呼ぶとき**（5 ターンごとに介入、直前の話者を名指し）:

| 規則 | 一巡 | 即時追従 | 応答待ち |
| --- | --- | --- | --- |
| `weighted`（採用） | 6 | 11/12 | 2（12 件・未応答 0） |
| `weighted`（介入時も ×0.2 を課す） | 5 | **0/12** | — |
| `weighted`（介入後は常に進行役） | 5 | **0/12** | — |
| `round_robin` | 4 | 0/12 | — |

読み取れたこと:

1. **呼びかけの係数には、意味のある帯がある。** 下限は `1/0.85 ≈ 1.176`（これ以下では直近発言の抑制が
   加点を打ち消し、×1.1 の即時追従率は 7/19 に落ちる）。上限は `1/(0.85 × 0.95) ≈ 1.238`（これ以上では
   最悪の組み合わせでも呼びかけ先が勝つので、窓が働く余地が消え、×1.5 と ×2.0 の結果が一致する）。
   採用した ×1.2 はこの帯の中にある。起票時の原案「呼びかけ先以外を ×0.5」は順位としては ×2.0 で、
   上限より上にあった。
2. **窓を入れると、応答されない呼びかけが無くなる。** 複数呼びかけで未応答 2/40 → **0/45**、
   名指しの無い介入があるシナリオで 1/20 → **0/20**。窓なしの形は呼びかけを末尾のメッセージ 1 つからしか
   読まないので、`[A, B]` の A が話した瞬間に B への呼びかけが視界から消えていた。
3. **即時追従率は下がるが、それは「即時」を測っているためである。** 複数呼びかけで 25/25 → 17/27、
   介入ありで 18/19 → 9/20 に落ちる一方、応答待ちの最大値は 4 ターンで未応答は 0 になる。
...
5. **応答で消える形が、残る形より測定上ほぼ全面的に優る。** 複数呼びかけで一巡 7 対 9・
   ...
   応答した参加者ではなく進行役で（37%）、それは窓なしの形（35%）でも同じである。
   連続発言の抑制 ×0.2 が直後の再登板を止め、直近発言の抑制 ×0.85 が加点の 1.2 を 1.02 まで削るためである。
6. **窓を与えた順位規則とは互角で、傾向が違う。** `addressed_first`（窓 4・未応答・沈黙が長い側）は
   即時追従率で勝ち（複数呼びかけで 27/29 対 17/27）、`weighted` は一巡ターン数（7 対 10）と
   未応答（0 対 1）と最大シェア（33% 対 35%）で勝つ。順位規則では呼びかけが常に他を先取りするため
   巡回が伸び、`weighted` では呼びかけが他の圧力と競合するため巡回が保たれる。
7. **同点規則は依然として必須。** 窓を入れると複数呼びかけのシナリオでは同点が 0 になるが、
   呼びかけ 3 割と介入ありのシナリオでは 2 回残る。名簿順だけで割ると一巡ターン数が 12 まで伸びる。
8. **人間の介入発言の後に ×0.2 を課し続けると、名指しの介入が一度も届かない**（即時追従率 0/12）。
   人間の発言を現ターンの発言者とみなすと 11/12、応答待ち 2 ターン・未応答 0 になる（4.6.7）。
...
- **呼びかけは窓つきの加点で表す**（窓 = 編成の人数、×1.2、応答で消える）。起票時の原案「呼びかけ先以外を
  ×0.5」から替えた。**Why**: 原案は順位としては ×2.0 に当たり、呼びかけ先が必ず勝つうえ、末尾のメッセージ
  1 つしか読まないので応答されない呼びかけが残る（読み取り 1・2）。窓つきの ×1.2 は、未応答を 0 にしつつ
  一巡ターン数と発言の偏りも改善する（読み取り 4）。
...
**係数の帯は実装上の制約として扱う**（読み取り 1）。呼びかけの加点は `1.176 < x < 1.238` の帯を外れると
意味が変わる — 下回れば呼びかけが届かず、上回れば窓が働かなくなって原案と同じになる。
値を動かすなら帯の中に限り、動かした場合はこの節の測定をやり直す。
...
- **(b) 名前の照合（(a) が空振りしたときだけ）**: 発言の最終文に現れた編成の表示名のうち、自分自身を除いた
  ...
  最終文への限定と 2 文字以上の条件がその抑えである。**編成の全員が一致した場合は呼びかけ無しとする**
  （「みなさん」に相当する発言で全員が ×0.5 を免れると、呼びかけが無いのと同じ重みの並びになり、
  検出したことが結果に何も足さないため）。
...
（4.6.3 の 2 番目の表）、上書きで得られるものが無くなった。
...
| C の係数 | 一巡ターン数 | 応答待ち | 同点 |
| --- | --- | --- | --- |
| 1.0（係数なし = 現行） | 7 | 1（未応答 0） | 2 |
| 0.8 | 22 | 1（12 件・未応答 0） | 2 |
| 0.5 | 21 | 9（未応答 1/10） | 3 |
| 0.2 | **一巡せず** | 1（**未応答 8/18**） | 1 |

0.2 の C は 60 ターンのうち一巡に加わらない。「控えめ」を設定した利用者が得るのは「無言」であり、
...
進行役に係数 1.15 を与えると、4 人（GM + 3）の編成で現行と全指標が一致する（進行役の間隔 1〜3、
