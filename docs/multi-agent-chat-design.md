...
- **既存 chat へのプリセット適用の可否と既存参加者の扱い → 発言 0 件に限って許可し、編成は置換**（2026-09-20）:
  ...
  適用はターンエンジンの chat 単位の排他（§4.2 手順 0）の中で行い、発言の有無もその中で読む
  （`TurnEngine.WithTurnExcluded`）。**Why**: ターンは発言者を選んでから完了時に message を保存するので、
  排他の外で消すと、保存された message が既に消えた参加者を指し、話者を解決できない発言が残る。
  ターンが走っているあいだの適用は 409。
