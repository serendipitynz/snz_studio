...
**名指しの無い介入のときだけ進行役を強制する上書きも測ったが、入れない。** 名指しのある介入では
上書きが働かないので追従率は落ちない（12/12 のまま）が、規則が「重みで決める」一点では済まなくなる。
名指しの無い介入での差は一巡ターン数と進行役の間隔に出るが、窓を入れた後は採用案の側が一巡 5 で締まっており
（4.6.3 の 2 番目の表）、上書きで得られるものが無くなった。名指しのある介入は上書きの対象外なので
即時追従率は 11/12 のまま変わらず、**得も損もしない**。規則を「重みで決める」一点に保つほうを採る。
それでも入れないのは、進行役が居る編成では直近発言の抑制の免除によって進行役がすでに応じやすく
（4.6.3 読み取り 6）、上書きはその傾向を確定に変えるだけで、確定させたいなら
`facilitator_alternating` を選べば得られるものだからである。
