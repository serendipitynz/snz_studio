...
**呼びかけ先が 1 人**（呼びかけ 3 割）。呼びかけは全規則で同じ 21 件起き、応答待ちの分母はそのうち
打ち切り位置の補正（4.6.2）を通った 19 件である。応答待ちの欄で件数だけを書いてあるものは未応答 0 を意味する:
...
**呼びかけ先が複数**（呼びかけ 4 割・うち半分が 2 人呼び。呼びかけ先は全規則で 26 件、補正後 39 組）:
...
違わず、効くのは呼びかけ先が複数のとき（最大発言シェア 32% 対 40%、複数呼びかけ待ち 4 対 6）と、
...
- **(b) 名前の照合（(a) が空振りしたときだけ）**: 発言の最終文に現れた編成の表示名のうち、自分自身を除いた
  ...
  測定上の差は小さい（4.6.3 の最後の段落: 全員呼びでは加点の有無で結果がほとんど変わらない）が、
  同じではないので、根拠の無い特別扱いを置かない。
