...
one step. Twenty-five presets ship with the app; others are applied by loading a JSON file of the
same shape (format: `docs/multi-agent-presets.md`). A preset can be applied when the
...
are a draft, sent with "Save configuration".

- A stored theme family or mode this build does not have is drawn as Standard + Auto (follow OS) and left as it is;
