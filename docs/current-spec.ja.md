...
プロジェクトのドキュメント・メモリをプロジェクト資料として読みますが、発言のメモリ保存（§6.5）を受け付けません
...
- 参加者: 表示名・役割プロンプト・接続先・モデル・編成順・プロジェクト資料を渡すか（既定は渡す）
- ドキュメント・メモリ: 「全参加者に渡す」（`shared_with_all`）。ドキュメントの既定は偽、メモリは発言から保存したもの
  （`source = multi_agent`）だけ真
- ターン進行ルール: `round_robin`（編成順の循環）/ `manual`（発言者の指名）/
  `facilitator_alternating`（進行役と他の参加者が交互。他の参加者は編成順に循環）/
  `weighted`（発言が呼びかけた参加者や長く黙っていた参加者が次に話し、同じ参加者は続けて話さない）
- 進行役: `facilitator_alternating` が挟む参加者 1 人。最初のターンと人間の介入発言には進行役が応じます。
  進行役が編成にいないあいだは編成順の循環に落ちます。`weighted` では任意で、直近に話したことで順番を
  下げられない扱いを受けるだけです
- 呼びかけ: 発言を保存するときに、その発言が呼びかけた参加者を一緒に記録します。`weighted` で参加者が最後の行に書く
  `[次: 表示名]`（保存する本文からは除きます）、無ければ発言の最終文に現れた編成の表示名が呼びかけ先です。
  人間の介入発言も同じ照合にかけるので、「アリス、もう少し詳しく」は `weighted` でアリスに届きます
- 場面設定: 全参加者のシステムプロンプトに前置される論題・シーン・世界観

**編成**とは、除籍されていない参加者の集合を指します（`round_robin` の巡回対象・インスペクタの編成の表示対象）。
除籍は論理削除なので行は残り、過去の発言は表示名を保ち、巡回の起点も失われません。

**プリセット**は、参加者・ターン進行ルール・場面設定（と、プリセットが印を付けていれば進行役）を
一度で埋める雛形です。25 件をアプリに
同梱し、それ以外は同じ形式の JSON ファイルを読み込んで適用します（形式は `docs/multi-agent-presets.md`）。
適用できるのは新規作成時と、発言がまだ 1 件も無い多人数会話に対して（インスペクタから）です。後者は既存の
編成・ターン進行ルール・場面設定を置き換えます。発言が 1 件でもある会話には適用できません（transcript が
既に居ない話者を指すため）。適用後の chat はプリセットとの結びつきを持たず、以後の編集はすべてインスペクタから行います。
...
2. 発言者を決める（`round_robin`: `participant_id` を持つ直近メッセージの次の編成順 / `manual`: 指名された参加者 /
   `facilitator_alternating`: 直前が進行役なら進行役以外で最後に発言した参加者の次、それ以外は進行役 /
   `weighted`: 参加者ごとの重み（直前の話者 ×0.2・直近 n 発言のうちに話した参加者 ×0.85（進行役を除く）・
   未応答の呼びかけ ×1.2 の積）が最大の参加者。並んだら長く黙っている側、なお並べば編成順）
3. 参加者の接続先を確認し、モデルのロードを確認したうえで、話者をクライアントへ通知する（`speaker` イベント。
   参加者・解決後のモデル名、重みで選ぶ規則なら重みの内訳を載せる）。ここまでの拒否は HTTP ステータス
   （409 / 404 / 400 / 502）で返り、これより後の失敗はストリーム内の `error` イベントになる
4. 発言者本人の視点でプロンプトを組む。system = プロジェクト資料（プロジェクト説明・検索で得たドキュメント抜粋・
   関連メモリ。後述）+ 場面設定 + 役割プロンプト + 役割リマインド。履歴は自分の
   過去発言を `assistant`、他の参加者と人間の発言を `user`（「表示名: 本文」の形）へ写像し、末尾はその参加者が
   答えるべき直前の発言にする
5. 発言を stream し、`participant_id` を持つ `assistant` message として、プロジェクト資料の元になった参照と
   1 トランザクションで保存する
...
引用モード・他チャット参照）は使いません。プロジェクト資料はターンごとに 1 回、最新発言 + 直前 2 発言 + 場面設定の先頭
...
ターンは失敗せず、プロジェクト資料なしで続行します。プロジェクト資料を渡さない参加者が発言するターンでは、
「全参加者に渡す」を付けたドキュメント・メモリ（共通プロジェクト資料）だけを渡し、プロジェクト説明は渡しません。
絞り込みは検索の前（SQL 側）で行うので、ターンの少ない予算が渡らない資料に食われません。
プレイヤーがルール・世界観を読みつつ GM だけがシナリオを知る編成のためで、「全参加者に渡す」の既定は偽です。
...
複数の発言にまたがる結論は、ヘッダの「結論を要約」（会話全体）か発言の「ここから要約」（その発言から最新まで）で
既定 LLM に下書きさせ、同じダイアログに入れて保存します。

- 下書きは「決定」「未決」を分けて書かれ、役割に沿って出た反論は他の参加者の同意か人間の採用が無い限り決定に入らない
- 既定の kind は semantic。保存するまで何も永続化されず、`chat_summaries` にも書かない
- 範囲が 12000 字を超えると生成せず、もっと後の発言から要約するよう促す（先頭を切り捨てない）
- 一時チャットでも下書きは作れるが、保存はできない
...
- 保存されている配色系統・明暗がこの版に無いときは、標準・自動 (OS に従う) で描き、保存値は書き換えずに
  外観の区画で案内します (閉じられる。モーダルを開き直すとまた出る)。ここで選び直すと置き換わります。
- 配色の選択を保存できなかったときは、その起動の間は効かせ、保存できなかったことを外観の区画に失敗として出します。
- 各エンドポイントの接続の状態は、欄の名の横に語 (「接続済み」/「未接続」) で出します。保存済みの値に対する状態なので、
  下書きでそのエンドポイント・モデルを書き換えている間は出しません。画像説明用エンドポイントは、画像説明用モデルが保存されているときだけ出します。
- 接続の下書きが保存値と違うまま閉じる (閉じるボタン・Escape) と、破棄前確認を出します。既定は「編集を続ける」。
  保存中は閉じません。読み込み中に空でない値を書いた欄は、読み込みの結果で上書きしません。
  読み込みに失敗したときは保存できません (書いていない欄が空で保存されるため)。
- 内蔵の埋め込みモデルの取得中は、転送量と割合の進捗を出します。量が分かるまでは流れる印の進捗です。
...
- 最近のチャット: 全 project のチャットを更新の新しい順に最大 10 件。project 名と日時を添え、押すとそのチャットを開きます
...
- 接続状態: チャット・レビュー・埋め込み・画像説明の接続先に届くかをモデル名とともに表示し、設定を開くボタンを置きます。設定を閉じると確認し直します。内蔵の埋め込みモデルは接続先の代わりに状態 (利用可能・準備中・利用できません) を、モデル未設定の画像説明は「未設定」を表示します
- 並べ替えても各 project の最終更新は変わりません
...
右のプロジェクトのアセットは、チャット画面のコンテキストインスペクタと同じ形で見出し帯のボタンから
出し入れします（§11.4。保存キーは画面ごとに別）。
...
- context inspector（見出し帯のボタンで出し入れする。広い画面では列として出し、出し入れを localStorage に
  保存する。1180px 以下では、保存した選択を変えずに消えた状態から始まり、ボタンを押すと会話の右端に画面の端から滑り込んで重なる (動きを減らす設定ではフェードだけ)。
  重ねた区画は自分の閉じるボタンを持ち、焦点を閉じ込めない。Escape と閉じるボタンで閉じ、区画に隠れた部品へ
  焦点が移ったときも閉じる）
...
### 11.6 多人数会話画面（観戦ビュー + インスペクタ）

中央:

- 発言ごとに発言者の表示名とモデル名を付けた transcript
- 進行中ターンの stream 表示。話者名はサーバーの `speaker` イベントで付ける
- 「1 ターン進める」/「自動進行の開始・停止」/（`manual` のとき）次の発言者の指名 /
  （`facilitator_alternating` で進行役が編成にいないとき）その旨の注記
- 人間として会話に発言する composer
- 発言ごとの「メモリに保存」（§6.5）。一時チャットでは無効になり、理由を composer の注記に出す
- ヘッダの「結論を要約」と発言ごとの「ここから要約」（§6.5）

右:

- インスペクタ: 折り畳める区画の並び。開閉の既定は会話が始まる前と後で変わり、最初の発言が付いた時点で
  切り替わります（状態を開き、会話の設定と参加者のカードを畳みます）。開閉の選択は保存しません。
  - プリセット: 発言が 1 件も無いあいだだけ出ます。畳みが既定
  - 状態: 共通の状態と、編成にいる参加者ごとの状態シート（欄の名は「表示名の状態」で表示名は太字、編成順。
    字数カウンタは欄の名の行の右端、保存は右寄せ）。会話が始まる前は畳み、
    始まった後は展開が既定。ターンの実行中・自動進行中も編集と保存ができます
  - 会話の設定: ターン進行ルール、進行役（`facilitator_alternating` か `weighted` を選んだときだけ出ます）、
    場面設定。会話が始まる前は展開、始まった後は畳みが既定
  - 編成: 参加者ごとの折り畳みカード（会話が始まる前は展開、始まった後は畳みが既定。追加した直後のカードは
    開きます）。上下移動と退席はカードの見出しにあり、畳んだままでも並べ替え・退席ができます。中身は
    表示名・役割プロンプト・「プロジェクト資料を渡す」とその保存、区切り線を挟んで、接続先・接続確認・
    モデルとその保存

除籍済みの参加者は編成とは別に一覧します（過去の発言が残るため）。
インスペクタは、チャット画面のコンテキストインスペクタと同じ形で見出し帯から出し入れします（§11.4）。
