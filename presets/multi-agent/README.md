...
フォームの「プリセットの JSON を読み込む」からファイルを選ぶと同じように適用できます。
...
読み込んだ JSON は `POST /api/projects/{projectId}/chats` の body に `preset` として送られ、
同梱分 (`presetId`) と同じ検証・適用処理を通ります。適用後のチャットはプリセットとの結びつきを持たず、
